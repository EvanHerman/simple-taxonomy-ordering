			...
			tax_table.sortable({
				...
				start: function( event, ui ) {
					...
					item.siblings().each( function() {
						...
						if ( ele.attr( 'id' ) !== item.id && eleIndex > index && halt === false ) {
							if ( parseInt( ele.find( '.parent' ).text() ) === 0 ) {
								halt = true;
							} else {
								ele.find( 'input[type="checkbox"]' ).hide().after( '<img src="' + simple_taxonomy_ordering_data.preloader_url + '" class="yikes-simple-taxonomy-preloader" />' );
								ele.addClass( 'sto-child-term' );
							}
						}
					});
				},
				stop: function( event, ui ) {
					console.log( $( '.yikes-simple-taxonomy-preloader' ).not( $( ui.item[0] ).find( '.yikes-simple-taxonomy-preloader' ) ).prev( 'input[type="checkbox"]') );
					console.log( $( '.yikes-simple-taxonomy-preloader' ).not( $( ui.item[0] ).find( '.yikes-simple-taxonomy-preloader' ) ).prev( 'input[type="checkbox"]').next( '.yikes-simple-taxonomy-preloader' ) );
					$( '.yikes-simple-taxonomy-preloader' ).not( $( ui.item[0] ).find( '.yikes-simple-taxonomy-preloader' ) ).prev( 'input[type="checkbox"]').show().next( '.yikes-simple-taxonomy-preloader' ).remove();
				},
				// Update callback.
				update: function( event, ui ) {
					...
					tax_table.find( 'tr.ui-sortable-handle' ).each( function() {
						...
						if ( ele.hasClass( 'sto-child-term' ) ) {
							ele.detach().insertAfter( '.sto-parent-term' );
						}

						// ele.removeClass( 'sto-child-term sto-parent-term' );
